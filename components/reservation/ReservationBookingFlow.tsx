...
export function ReservationBookingFlow({ embedded = false }: { embedded?: boolean }) {
  ...
  if (step === "confirmed") {
    ...
    if (embedded) {
      return <div className="flex justify-center py-2 animate-fade-up">{confirmed}</div>
    }
    return (
  ...
  if (step === "details") {
    const detailsInner = (
        <div className="mx-auto max-w-3xl flex-1 px-0 py-4 sm:px-2 sm:py-6">
          {embedded ? (
            <Button type="button" variant="ghost" size="sm" className="mb-4 -ml-1" onClick={() => setStep("select")}>
    ...
    if (embedded) {
      return <div className="animate-fade-up">{detailsInner}</div>
    }
    return (
      <PageShell contentClassName="pb-20 lg:pb-0">
  ...
  const selectInner = (
      <div className={embedded ? "py-2" : "site-container flex-1 py-10"}>
        {embedded ? (
          <div className="mb-6">
            <h2 className="font-display text-2xl font-semibold text-amber-950">Réserver une table</h2>
            <p className="mt-1 text-sm text-amber-900/65">
              Choisissez date, heure et emplacement — nous préparons votre accueil.
            </p>
          </div>
        ) : null}
        {isReturningCustomer && (
  ...
  if (embedded) {
    return (
      <div className="max-h-[min(88vh,940px)] overflow-y-auto overflow-x-hidden pr-1 animate-fade-up">
        {selectInner}
      </div>
    )
  }
