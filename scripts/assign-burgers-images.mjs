#!/usr/bin/env node
/**
 * Upload Burgers product images → Supabase Storage + update products.image_url
 *
 * Usage:
 *   node --env-file=.env.local scripts/assign-burgers-images.mjs
 */
...
const SRC_DIR = join(ROOT, "data/menu-images/burgers")
...
async function uploadAndLink(slug, buf) {
  const objectPath = `products/burgers/${slug}.webp`
...
console.log("🍔  Burgers — upload images menu\n")
let ok = 0
for (const slug of BURGER_SLUGS) {
  try {
...
console.log(`\nTerminé: ${ok}/${BURGER_SLUGS.length} produits mis à jour.`)
