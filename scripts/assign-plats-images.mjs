#!/usr/bin/env node
/**
 * Upload Plats product images → Supabase Storage + update products.image_url
 *
 * Usage:
 *   node --env-file=.env.local scripts/assign-plats-images.mjs
 */
...
const SRC_DIR = join(ROOT, "data/menu-images/plats")
const BUCKET = "menu-product-images"

const PLATS_SLUGS = [
  "crispy-chicken-teller",
  "crispy-zinger-teller",
  "fajita-teller",
  "mexicano-teller",
  "falafel-halloumi-teller",
  "halloumi-teller",
  "arabischer-falafel-teller",
  "chicken-fries",
  "frittierter-fisch-teller",
]
...
async function uploadAndLink(slug, buf) {
  const objectPath = `products/plats/${slug}.webp`
...
console.log("🍽️  Plats — upload images menu\n")
let ok = 0
for (const slug of PLATS_SLUGS) {
  try {
...
console.log(`\nTerminé: ${ok}/${PLATS_SLUGS.length} produits mis à jour.`)
