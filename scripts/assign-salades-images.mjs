#!/usr/bin/env node
/**
 * Upload Salades product images → Supabase Storage + update products.image_url
 *
 * Usage:
 *   node --env-file=.env.local scripts/assign-salades-images.mjs
 */
...
const SRC_DIR = join(ROOT, "data/menu-images/salades")
const BUCKET = "menu-product-images"

const SLUGS = ["rucola-salat", "tabbouleh", "fattoush", "salat"]
const SIZE = 1200
...
async function uploadAndLink(slug, buf) {
  const objectPath = `products/salades/${slug}.webp`
...
console.log("🥗  Salades — upload images menu\n")
let ok = 0
