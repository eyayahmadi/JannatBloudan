#!/usr/bin/env node
/**
 * Upload Manakish product images → Supabase Storage + update products.image_url
 *
 * Usage:
 *   node --env-file=.env.local scripts/assign-manakish-images.mjs
 *
 * Expects square-ready sources in data/menu-images/manakish/{slug}.png
 * Outputs 1200×1200 WebP to Supabase bucket menu-product-images/products/manakish/
 */
...
const SRC_DIR = join(ROOT, "data/menu-images/manakish")
const BUCKET = "menu-product-images"

const MANAKISH_SLUGS = [
...
async function uploadAndLink(slug, buf) {
  const objectPath = `products/manakish/${slug}.webp`
...
console.log("🫓  Manakish — upload images menu\n")
let ok = 0
for (const slug of MANAKISH_SLUGS) {
  try {
...
console.log(`\nTerminé: ${ok}/${MANAKISH_SLUGS.length} produits mis à jour.`)
